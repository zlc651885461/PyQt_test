...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        self.tableWidget.setStyleSheet("border:2px solid gray;"
        ...
        self.ABCImg.setPixmap(QtGui.QPixmap(os.path.join('./resource/LabelABC.png')))
    ...
    def clock(self):
        ...
        if not self.flag_start:
            ...
            dalte_t = datetime.now().replace(microsecond=0) - self.start_time
    ...
    def openfile(self):
        # 获取路径
        openfile_name, openfile_type = QFileDialog.getOpenFileName(self, '选择文件', os.path.join('C://'), 'Excel files(*.xlsx , *.xls);;WPS files(*.et,*.ett);; All files(*);')
    ...
    def item_alter(self,i,j):
        try:
            time_point = datetime.now().replace(microsecond=0)
            time_point_li = datetime.strftime(time_point, '%H:%M:%S').split(':')
            time_li = str(time_point - self.start_time).split(',')[-1].split(':')
            time_complete = ''.join(['用时:', time_li[-2], '分,', time_li[-1], '秒。'])
            time_point_str = ''.join(['-->完成时间:',time_point_li[0]+'时',time_point_li[1]+'分',time_point_li[2]+'秒;'])
    ...
    """
    def save_event(self):
        directory1 = QFileDialog.getSaveFileName(None, "文件保存", os.path.join('C://'), "Excel files(*.xlsx , *.xls);;WPS files(*.et,*.ett);;All files(*);")
